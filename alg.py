...
def find_funcs(G, P_1, P_2, C_s, C_1, C_2, E_s, E_1, E_2, S, p_1, p_2):
    ...
    while i <= len(S) - 1 and j <= len(S) - 1:
        ...
        if j == len(S) - 1:
            i = i + 1
            continue
        i_valid = np.isclose(G[i, j], P_2[i] * (P_1[j] * t + C_1[j] * E_1[j] - C_1[j - 1] * E_s[j]) * C_s[j] + G[i + 1, j])
        j_valid = np.isclose(G[i, j], P_1[j] * (P_2[i] * t + C_2[i] * E_2[i] - C_2[i - 1] * E_s[i]) * C_s[i] + G[i, j + 1])
        if not i_valid or not j_valid:
            print(S[i], S[j], i_valid, j_valid)
            print(P_1[j] * (P_2[i] * t + C_2[i] * E_2[i] - C_2[i - 1] * E_s[i]) * C_s[i] + G[i, j + 1] - P_2[i] * (P_1[j] * t + C_1[j] * E_1[j] - C_1[j - 1] * E_s[j]) * C_s[j] + G[i + 1, j])
...
def create_dists_grid():
    S_s = np.array([0.5]*SAMPLE_SIZE)
    S_1 = np.logspace(-1, 0.0, SAMPLE_SIZE)
    S_2 = 1.0 - np.logspace(-1, 0.0, SAMPLE_SIZE)
...
def create_dists_disc():
    S_s = {0.0: 1.0}
    S_1 = {0.1: 0.1, 0.3: 0.1, 0.5: 0.1, 0.9: 0.1}
    S_1 = S_1 | {k: 0.6 / SAMPLE_SIZE for k in np.arange(0.6, 0.8, 0.2 / SAMPLE_SIZE)}
    S_2 = {0.1: 0.1, 0.3: 0.1, 0.5: 0.1, 0.9: 0.1}
    S_2 = S_2 | {k: 0.6 / SAMPLE_SIZE for k in np.arange(0.6, 0.8, 0.2 / SAMPLE_SIZE)}
    print(np.sum(list(S_1.values())), np.sum(list(S_2.values())))
